<link id="css" rel="stylesheet" href="{{asset('css/components/header.css')}}">
    ...
    <div id="logoAreaCliente">
        <a id="linkLogo" href="{{route('index')}}"><img id="logoHeader"src="/img/logos/LogoSustent.svg" alt="LogoSustent"></a>

        <div id="containerLoginHeader">

            @if (auth()->guest())

                 <a href="{{route('login')}}"><button id="btn-loginHeader"type="button" >Login</button></a>

                @else
                <a id="perfilHeader" href="{{route('perfil')}}">
                ...
                <a id="sairHeader" href="{{route('auth.logout')}}">/ Sair</a>
            @endif
        </div>
    </div>
